    ...
    public class Node
    {
        ...
        bool _setupThing(Thing thing)
        {
            try
            {
                ...
                {
                    DebugEx.Assert("No thingkey specified for thing");
                    return false;
                }
                else
                {
        ...
        public bool AddThings(IEnumerable<Thing> things, bool SendToCloud = true) { return AddThings(things, null, SendToCloud: SendToCloud); }
        public bool AddThings(IEnumerable<Thing> things, INodeModule owner, bool SendToCloud = true)
        {
            bool error = false;
            try
            {
                if (things != null)
                    foreach (var thing in things.ToList())
                        try { AddThing(thing, SendToCloud: SendToCloud, owner: owner); } catch { error |= true; }
            }
            catch { error |= true; }
            return !error;
        }
        ...
        public Thing AddThing(Thing thing, bool SendToCloud = true) { return AddThing(thing, null, SendToCloud: SendToCloud); }
        public Thing AddThing(Thing thing, INodeModule owner, bool SendToCloud = true)
        {
            try
            {
                lock (locker)
        ...
        void _reConnectWithWorker()
        {
            if (AutoReconnect)
            {
                Task.Run(() =>
                {
                    Task.Delay(AutoReconnectDelay).Wait();
        ...
        public void HandleWarlockApiMsg(object msg)
        {
            try
            {
                if (msg is Yodiwo.API.Warlock.ShareNotifyMsg)
                {
                    var rsp = msg as Yodiwo.API.Warlock.ShareNotifyMsg;
                    try { OnReceivedShareNotifyMsg?.Invoke(rsp); }
                    catch (Exception ex) { DebugEx.TraceError(ex, "OnReceivedWarlockNotifyMsg failed"); }
                }
                else
                    try { OnUnexpectedMessage?.Invoke(msg); } catch (Exception ex) { DebugEx.TraceError(ex, "UserCode exception caught"); }
            }
                ...
                if (this._IsPaired && this.NodeKey != null)
                {
                    if (this.NodeKey.IsValid)
                    {
                        ...
                        if (Channel != null && (Channel.RemoteHost != ypserver || Channel.RemotePort != port.ToStringInvariant()))
                        {
                            ...
                            Channel.Close("Node disconnection (2)");
                            Channel = null;
                        }
                        ...
                        if (Channel.IsOpen)
                            Channel.Close("Node disconnection (3)");
                        //connect
                        _ypcConnecting = true;
                        try
                        {
