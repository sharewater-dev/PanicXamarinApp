...
namespace Yodiwo.YPChannel
{
    public abstract class Channel
    {
        ...
        public TimeSpan KeepAliveSpinDelay = TimeSpan.FromMinutes(10);
        ...
        public TimeSpan InactiveLineWarmupTimeout = TimeSpan.FromMinutes(5);
            ...
            try
            {
                while (State != ChannelStates.Closed)
                {
                    ...
                    lock (keepAliveTaskSyncLock)
                        Monitor.Wait(keepAliveTaskSyncLock, (int)KeepAliveSpinDelay.TotalMinutes.ClampFloor(1) * 60 * 1000);
                }
            }
            ...
            lock (syncRoot)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        onClose(Message);
                        //raise event
                        OnClosedEvent?.Invoke(this, Message);
                    }
        ...
        internal void HeartBeatEntry()
        {
            ...
            try
            {
                ...
                while (IsOpen || IsNegotiating)
                {
                    ...
                    //timestamp
                    if (msg.Payload is PongResponse)
